locals {
  definitions = {
    ...
    validator_expressions = {
      ...
      "site_config.ip_restriction_action"                       = "^(Allow|Deny)$"
      "site_config.ip_restriction_ip_address"                   = "^(\\d{1,3}\\.){3}\\d{1,3}(/\\d{1,2})?$"
      "site_config.scm_ip_restriction_action"                   = "^(Allow|Deny)$"
      "site_config.scm_ip_restriction_ip_address"               = "^(\\d{1,3}\\.){3}\\d{1,3}(/\\d{1,2})?$"
      "site_config.auto_heal_setting_trigger_status_code_range" = "^[1-5][0-9][0-9](-[1-5][0-9][0-9])?$"
      "auth_settings_v2_auth_enabled"                           = "^(true|false)$"
      "auth_settings_v2_require_authentication"                 = "^(true|false)$"
      "auth_settings_v2_unauthenticated_action"                 = "^(RedirectToLoginPage|AllowAnonymous)$"
      "auth_settings_v2_require_https"                          = "^(true|false)$"
      "auth_settings_v2_forward_proxy_convention"               = "^(NoProxy|Standard|CustomHeader)$"
    }
    ...
    validator_error_messages = {
      ...
      "site_config.ip_restriction_action"                       = "The action for ip_restriction must be either Allow or Deny."
      "site_config.ip_restriction_ip_address"                   = "The ip_address must be a valid IPv4 address or CIDR."
      "site_config.scm_ip_restriction_action"                   = "The action for scm_ip_restriction must be either Allow or Deny."
      "site_config.scm_ip_restriction_ip_address"               = "The ip_address for scm_ip_restriction must be a valid IPv4 or CIDR."
      "site_config.auto_heal_setting_trigger_status_code_range" = "The status_code_range must be a valid HTTP code or a range like 500-599."
      "auth_settings_v2_auth_enabled"                           = "The value must be true or false."
      "auth_settings_v2_require_authentication"                 = "The value must be true or false."
      "auth_settings_v2_unauthenticated_action"                 = "Must be one of: RedirectToLoginPage or AllowAnonymous."
      "auth_settings_v2_require_https"                          = "Must be true or false."
      "auth_settings_v2_forward_proxy_convention"               = "Must be one of: NoProxy, Standard, or CustomHeader."
    }
  }
}
